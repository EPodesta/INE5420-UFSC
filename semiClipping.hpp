...
class drawable {
    public:
        ...
        bool poligon_clipping(int wid, int hei) {
            auto beginVp = std::begin(vp);
            std::list<coord> coords;
            // coords.assign(vp.begin(), vp.end());
            auto it = vp.begin(), end = --std::end(vp);
            for (auto& i : vp) {
                std::cout << "Vp Before: " << i << std::endl;
            }
            drawable *d;
            bool clippedSource = false, clippedTarget = false;
            while(it != end){
                // std::cout << "Execs..." << std::endl;
                auto currentIt = it;
                it++;
                d = new drawable("dummy", {coord((*currentIt).x, (*currentIt).y), coord((*it).x, (*it).y)});
                if(d->line_clipping(wid, hei)) {
                    for (auto&i : d->vp) {
                        std::cout << "D Coords: " << i << std::endl;
                    }
                    auto iterate = std::begin(d->vp), endIt = --std::end(d->vp);
                    // if (std::find (coords.begin(), coords.end(), *iterate) != coords.end()) {
                    coords.emplace_back(*iterate);
                    std::cout << "Iterate: " << *iterate << std::endl;
                    if((*currentIt).x != (*iterate).x || (*currentIt).y != (*iterate).y) {
                        // std::cout << "CurrentIt is different!" << std::endl;
                        clippedSource = true;
                        // coords.erase(currentIt);
                        // currentIt = ind;
                        // for (auto&i : coords) {
                    ...
                    // if (std::find (coords.begin(), coords.end(), *iterate) != coords.end()) {
                    coords.emplace_back(*iterate);
                    std::cout << "IterateSecond: " << *iterate << std::endl;
                    if((*it).x != (*iterate).x || (*it).y != (*iterate).y) {
                        // std::cout << "it is different!" << std::endl;
                        clippedTarget = true;
                        // coords.erase(it);
                        // it = ind;
                        // for (auto&i : coords) {
            ...
            auto coordsIt = coords.begin(), coordsEnd = --coords.end();
            // auto itVp = vp.begin();
            // while (coordsIt != coordsEnd) {
                // if((*it).x > wid-10 || (*it).x < 10 || (*it).y > hei-10 || (*it).y < 10){
                    // auto ind = vp.insert(itVp, *coordsIt);
                    // itVp = vp.erase(itVp);
                    // coordsIt++;
                // } else {
                    // itVp++;
            ...
            //compute last two pairs
            d = new drawable("dummy", {coord((*it).x, (*it).y), coord((*beginVp).x, (*beginVp).y)});
            if(d->line_clipping(wid, hei)) {
                auto iterate = std::begin(d->vp), endIt = --std::end(d->vp);
                if ((*it).x != (*iterate).x || (*it).y != (*iterate).y) {
                    auto ind = coords.erase(coordsEnd);
                    coords.insert(ind, *iterate);
                    for (auto&i : coords) {
                        std::cout << "Last Point Coords: " << i << std::endl;
                    }
                }
                iterate++;
                if ((*beginVp).x != (*iterate).x || (*beginVp).y != (*iterate).y) {
                    auto ind = coords.erase(coordsIt);
                    coords.insert(ind, *iterate);
                    for (auto&i : coords) {
                        std::cout << "First Point Coords: " << i << std::endl;
                    }
                }
            }
            ...
            if (clippedSource || clippedTarget) {
                vp = coords;
            }
            // vp = coords;
            return true;
