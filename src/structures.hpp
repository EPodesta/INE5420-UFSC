...
public:
  explicit window(double width = 1, double height = 1,
                  double b = 1.0 / 20.0) noexcept : wid(width - b),
                                                    hei(height - b),
                                                    center(coord()) {}
...
public:
  ...
  std::list<coord> curve_clipping(const window &w) {
      ...
      drawable line("segment", {coord((*it).x, (*it).y), coord((*it++).x, (*it++).y)});
      ...
      // drawable *line;
      //
      std::cout << "Start Segment: " << (*std::begin(nSegment)).x << (*std::begin(nSegment)).y << std::endl;
      std::cout << "End Segment: " << (*(--std::end(nSegment))).x << (*(--std::end(nSegment))).y << std::endl;
      std::cout << "Line Start: " << (*std::begin(line.scn)).x << (*std::begin(line.scn)).y << std::endl;
      std::cout << "Line End: " << (*(--std::end(line.scn))).x << (*(--std::end(line.scn))).y << std::endl;
      while(it != end) {
        nSegment.clear();
        auto actualIt = it;
        it++;
        drawable line1("segment", {coord((*actualIt).x, (*actualIt).y), coord((*it).x, (*it).y)});
      ...
      std::cout << (*std::begin(coords)).x << (*std::begin(coords)).y << std::endl;
      return coords;
  }

  std::list<coord> liang_barsky(const window &w) {
    coord &s = *std::begin(scn), &t = *(--std::end(scn));
    std::vector<double> lu1({-1*(t.x - s.x), t.x - s.x, -1*(t.y - s.y), t.y - s.y}),
    ...
    if (RC1 != RC2) {
        if (!(RC1 & RC2)) {
            ...
            // if (((region_code(w, coord(centerX, centerY)) & 8) != 0
                ...
                // return emp;
            // }
            if((s.x > w.wid || s.x < -w.wid || s.y > w.wid || s.y < -w.wid) || (t.x > w.wid || t.x < -w.wid || t.y > w.wid || t.y < -w.wid)) {
                std::list<coord> emp;
                return emp;
            }
            aux = scn;
