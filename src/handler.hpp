...
extern "C" G_MODULE_EXPORT void btn_trans_figure_clk(GtkWidget *widget,
  ...
  double dt = sqrt((d.center()).y*(d.center()).y + (d.center()).z*(d.center()).z);
  ...
  std::unordered_map<int, matrix<double>> bases = {
      ...
      {2, m_transfer(-vector) * m_rotatex(a) * m_rotatey(-b) * m_rotatexyz(anglex, angley, anglez) * m_rotatey(b) * m_rotatex(-a) * m_transfer(vector)},
      {3, m_transfer(coord()) *  m_rotatex(a) * m_rotatey(-b) * m_rotatexyz(anglex, angley, anglez) * m_rotatey(b) * m_rotatex(-a)},
      {4, m_transfer(-d.center()) * m_rotatex(a) * m_rotatey(-b) * m_rotatexyz(anglex, angley, anglez) * m_rotatey(b) * m_rotatex(-a) * m_transfer(d.center())},
  };
